...
public class FunUtils {
    ...
    static <T, V> Function<T, Optional<V>> funSwitch(List<Tuple2<Predicate<T>, Function<T, V>>> list1) {
        return t -> list1.stream().dropWhile(tuple1 -> tuple1._1().negate().test(t)).findFirst().map(tuple -> tuple._2.apply(t));
    }

    static <T, V> Function<T, V> funSwitch2(List<Tuple2<Predicate<T>, Function<T, V>>> list1, Function<T, V> v2) {
